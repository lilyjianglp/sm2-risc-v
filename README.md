...
|--------|----------------|------------------|--------|----------|
| 模乘 `fp_mul` | 371 | 209 | **1.77×** | 🔥 +43.7% |
| 模约减 `fp_reduce` | 343 | 249 | 1.38× | +27.4% |
| 模平方 `fp_sqr` | 358 | 275 | 1.30× | +23.1% |
| 模逆 `fp_inv` | 139,701 | 110,300 | 1.27× | +21.0% |
| 标量点乘 `[d]G` | 2,255,643 | 1,756,000 | 1.28× | +22.1% |
...
项目使用 Makefile 进行统一管理，
支持同时生成 纯 C 基线版本 与 RISC-V 汇编优化版本：
...
test_sm2_kex_c ：纯 C 实现（Baseline）

test_sm2_kex_rv ：RISC-V 汇编优化实现（Optimized）
## 3️⃣ 运行性能测试
make run
...
并输出详细的 CPU cycle 统计结果，用于性能对比分析。
...
功能等效性
所有优化算子均通过 100% 覆盖测试，
Benchmark 最终校验值 [sink] 完全一致。

侧信道防御
底层实现保持 常量时间（Constant-Time） 执行，
不引入任何数据相关的条件跳转，避免时序侧信道泄露。
## 👤 项目信息

作者：姜良萍 · 王奕文

研究方向：RISC-V / 国密算法 SM2 / 密码工程
