    ...
    */
    ...
    { uint64_t cyc[REPEAT_FP]; fp_t t; memset(&t,0x12,sizeof(t)); t.v[0]|=1;
      ...
      for (int r=0;r<REPEAT_FP;r++) {
          ...
          cyc[r]=(rdcycle()-s0)/(uint64_t)ITER_FP;
          sink_mem(&t,sizeof(t)); }
      print_stats("fp_mont_mul (C)", cyc, REPEAT_FP); }
    ...
    { uint64_t cyc[REPEAT_FP]; fp_t s; memset(&s,0x56,sizeof(s)); s.v[0]|=1;
      ...
      for (int r=0;r<REPEAT_FP;r++) {
          ...
          cyc[r]=(rdcycle()-s0)/(uint64_t)ITER_FP;
          sink_mem(&s,sizeof(s)); }
      print_stats("fp_mont_sqr (C)", cyc, REPEAT_FP); }
    ...
    { uint64_t cyc[REPEAT_FP]; fp_t xi,ri; memset(&xi,0x9a,sizeof(xi)); xi.v[0]|=1;
      ...
      for (int r=0;r<REPEAT_FP;r++) {
          ...
          cyc[r]=(rdcycle()-s0)/(uint64_t)ITER_INV;
          sink_mem(&ri,sizeof(ri)); sink_mem(&xi,sizeof(xi)); }
      print_stats("fp_mont_inv (C)", cyc, REPEAT_FP); }

    /* ── fp_mont_add ── */
    { uint64_t cyc[REPEAT_FP]; fp_t a2, b2;
      memset(&a2,0x11,sizeof(a2)); memset(&b2,0xAA,sizeof(b2));
      a2.v[0]|=1; b2.v[0]|=1;
      fp_to_mont(&a2,&a2); fp_to_mont(&b2,&b2);
      for (int r=0;r<REPEAT_FP;r++) {
          for (int i=0;i<200;i++) fp_mont_add(&a2,&a2,&b2);
          uint64_t s0=rdcycle();
          for (int i=0;i<ITER_FP;i++) {
              fp_mont_add(&a2,&a2,&b2);
              if((i&127)==0) sink_mem(&a2,sizeof(a2));
          }
          cyc[r]=(rdcycle()-s0)/(uint64_t)ITER_FP;
          sink_mem(&a2,sizeof(a2)); }
      print_stats("fp_mont_add", cyc, REPEAT_FP); }

    /* ── fp_mont_sub ── */
    { uint64_t cyc[REPEAT_FP]; fp_t a3, b3;
      memset(&a3,0x55,sizeof(a3)); memset(&b3,0x22,sizeof(b3));
      a3.v[0]|=1; b3.v[0]|=1;
      fp_to_mont(&a3,&a3); fp_to_mont(&b3,&b3);
      for (int r=0;r<REPEAT_FP;r++) {
          for (int i=0;i<200;i++) fp_mont_sub(&a3,&a3,&b3);
          uint64_t s0=rdcycle();
          for (int i=0;i<ITER_FP;i++) {
              fp_mont_sub(&a3,&a3,&b3);
              if((i&127)==0) sink_mem(&a3,sizeof(a3));
          }
          cyc[r]=(rdcycle()-s0)/(uint64_t)ITER_FP;
          sink_mem(&a3,sizeof(a3)); }
      print_stats("fp_mont_sub", cyc, REPEAT_FP); }

    /* ── fp_to_mont ── */
    { uint64_t cyc[REPEAT_FP]; fp_t a4, r4;
      memset(&a4,0x33,sizeof(a4)); a4.v[0]|=1;
      for (int r=0;r<REPEAT_FP;r++) {
          for (int i=0;i<200;i++) fp_to_mont(&r4,&a4);
          uint64_t s0=rdcycle();
          for (int i=0;i<ITER_FP;i++) {
              fp_to_mont(&r4,&a4);
              if((i&127)==0) sink_mem(&r4,sizeof(r4));
          }
          cyc[r]=(rdcycle()-s0)/(uint64_t)ITER_FP;
          sink_mem(&r4,sizeof(r4)); }
      print_stats("fp_to_mont", cyc, REPEAT_FP); }

    /* ── fp_from_mont ── */
    { uint64_t cyc[REPEAT_FP]; fp_t a5, r5;
      memset(&a5,0x77,sizeof(a5)); a5.v[0]|=1;
      fp_to_mont(&a5,&a5);
      for (int r=0;r<REPEAT_FP;r++) {
          for (int i=0;i<200;i++) fp_from_mont(&r5,&a5);
          uint64_t s0=rdcycle();
          for (int i=0;i<ITER_FP;i++) {
              fp_from_mont(&r5,&a5);
              if((i&127)==0) sink_mem(&r5,sizeof(r5));
          }
          cyc[r]=(rdcycle()-s0)/(uint64_t)ITER_FP;
          sink_mem(&r5,sizeof(r5)); }
      print_stats("fp_from_mont", cyc, REPEAT_FP); }
