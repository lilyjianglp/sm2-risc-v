...
static void sm2_curve_params(fp_t *a, fp_t *b) {
    fp_from_bytes(a,SM2_A_BYTES); fp_from_bytes(b,SM2_B_BYTES); }

int sm2_point_encode_uncompressed(uint8_t out[SM2_POINT_ENC_UNCOMPRESSED_SIZE], const sm2_affine_t *P) {
    if(!P||sm2_affine_is_infinity(P)) return 0;
    out[0]=0x04; fp_to_bytes(out+1,&P->x); fp_to_bytes(out+33,&P->y); return 1; }

int sm2_point_decode_uncompressed(sm2_affine_t *P, const uint8_t in[SM2_POINT_ENC_UNCOMPRESSED_SIZE]) {
    if(!P||!in||in[0]!=0x04) return 0;
    fp_from_bytes(&P->x,in+1); fp_from_bytes(&P->y,in+33); P->infinity=0;
    return sm2_point_is_valid(P); }

int sm2_point_is_valid(const sm2_affine_t *P) {
    if(!P||sm2_affine_is_infinity(P)) return 0;
    fp_t a,b; sm2_curve_params(&a,&b);
    fp_t y2,x2,x3,ax,rhs;
    fp_sqr(&y2,&P->y); fp_sqr(&x2,&P->x); fp_mul(&x3,&x2,&P->x);
    fp_mul(&ax,&a,&P->x); fp_add(&rhs,&x3,&ax); fp_add(&rhs,&rhs,&b);
...
int sm2_kex_compute_Z(uint8_t Z[32], const uint8_t *id, size_t id_len, const sm2_affine_t *Ppub) {
    ...
    fp_t a,b; sm2_curve_params(&a,&b);
    uint8_t a_bytes[32],b_bytes[32]; fp_to_bytes(a_bytes,&a); fp_to_bytes(b_bytes,&b);
...
static int sm2_compute_shared_point(sm2_affine_t *out_xy,
    const uint8_t t_self_be[32], const fn_t *x_other_dash,
    const sm2_affine_t *R_other, const sm2_affine_t *P_other)
{
    if(!out_xy||!t_self_be||!x_other_dash||!R_other||!P_other) return 0;
    if(!sm2_point_is_valid(R_other)||!sm2_point_is_valid(P_other)) return 0;
    ...
    sm2_affine_t temp_aff; sm2_jacobian_to_affine(&temp_aff,&J1);
 ...
 *  协议：普通域
 * ============================================================ */
int sm2_kex_initiator_gen_RA(sm2_affine_t *RA, const uint8_t rA[32]) {
    if(!RA||!rA) return 0;
    sm2_affine_t G; sm2_get_base_affine(&G);
    sm2_jacobian_t J; sm2_scalar_mul_window_ct_schemeB(&J,&G,rA);
    ...
    const uint8_t *idB, size_t idB_len, const sm2_affine_t *RB)
{
    ...
    sm2_affine_t V; if(!sm2_compute_shared_point(&V,tA,&x2d,RB,PB)) return 0;
    uint8_t xV[32],yV[32]; fp_to_bytes(xV,&V.x); fp_to_bytes(yV,&V.y);
    uint8_t kdf_in[128]; memcpy(kdf_in,xV,32); memcpy(kdf_in+32,yV,32);
    memcpy(kdf_in+64,ZA,32); memcpy(kdf_in+96,ZB,32);
    sm3_kdf(K,klen,kdf_in,128);
    ...
    const sm2_affine_t *PA, const uint8_t *idA, size_t idA_len)
{
    ...
    if(!sm2_point_is_valid(RB)) { if(!sm2_kex_initiator_gen_RA(RB,rB)) return 0; }
    uint8_t ZA[32],ZB[32];
    if(!sm2_kex_compute_Z(ZA,idA,idA_len,PA)) return 0;
    if(!sm2_kex_compute_Z(ZB,idB,idB_len,PB)) return 0;
    fn_t x1d,x2d; sm2_kex_x_dash(&x1d,&RA->x); sm2_kex_x_dash(&x2d,&RB->x);
    uint8_t tB[32]; if(!sm2_compute_t(tB,dB,rB,&x2d)) return 0;
    sm2_affine_t U; if(!sm2_compute_shared_point(&U,tB,&x1d,RA,PA)) return 0;
    uint8_t xU[32],yU[32]; fp_to_bytes(xU,&U.x); fp_to_bytes(yU,&U.y);
    uint8_t kdf_in[128]; memcpy(kdf_in,xU,32); memcpy(kdf_in+32,yU,32);
    memcpy(kdf_in+64,ZA,32); memcpy(kdf_in+96,ZB,32);
    sm3_kdf(K,klen,kdf_in,128);
